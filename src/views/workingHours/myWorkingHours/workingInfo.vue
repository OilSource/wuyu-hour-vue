    ...
    >
      <div class="infoOne">
        ...
        >
          ...
          <el-form :inline="false" label-width="300px">
            <el-form-item
              v-for="(item, index) in projectList"
              :key="index"
              :label="''"
            ...
            >
            项目名称：
            <el-select v-model="item.projectId" placeholder="选择项目" :disabled="item.approveStatus == 1">
              <el-option v-for="item in projectArr" :key="item.projectId" :label="item.projectName" :value="item.projectId"/>
            </el-select>
              <div style="display: flex; justify-content: flex-end;" >
                <span style="margin:0 5px">今日工时</span>
                <el-input
                  v-model.number="item.useHour"
                  type="number"
                  :max="24"
                  ...
                  placeholder="工时"
                  style="width:10%"
                  :disabled="item.approveStatus == 1"
                  @input="lookinput(item.useHour)"
                ></el-input>
                <span style="margin:0 5px">加班工时</span>
                <el-input
                  ...
                  @input="lookinput(item.useHour)"
                ></el-input>
                <span style="margin:0 5px">变更工时</span>
                <el-input
                  ...
                  @input="lookinput(item.useHour)"
                ></el-input>
                <span style="margin:0 5px">调休工时</span>
                <el-input
                  ...
                  @input="lookinput(item.useHour)"
                ></el-input>
                <span style="margin:0 5px">请假工时</span>
                <el-input
...
<script>
import { getMyHourDetailt, updateHour, getMyActorProject} from "@/api/system/project";
...
export default {
  ...
  methods: {
    open() {
      ...
      this.getprojectArr(this.fillDate);
      console.log(this.id);
      this.getmyhourdetail(this.id);
    ...
    addProjectHours(){
      ...
      this.projectList.push(
        {projectId:this.projectArr[0].projectId,hour:2,daily:''}
      )
    },
  }
};
