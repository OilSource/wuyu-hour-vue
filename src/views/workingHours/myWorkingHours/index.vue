  ...
  <div class="main">
          ...
          <el-card class="box-card">
            ...
            <div class="text item">
              ...
              >
                ...
                <el-table-column prop="totalHour" label="已批工时">
                  <template slot-scope="scope">
                    <span :style="'color:'+(scope.row.totalHour == scope.row.fillHour?'black':'red')"> {{ hourFormat(scope.row.totalHour) }}</span>
                  </template>
                </el-table-column>
                <el-table-column prop="fillHour" label="已报工时">
                  <template slot-scope="scope">
                    <span :style="'color:'+(scope.row.fillHour<8?'red':'black')"> {{ hourFormat(scope.row.fillHour) }}</span>
                  </template>
                </el-table-column>
                <el-table-column
                  prop="createTime"
    ...
    >
      <el-form :model="form" status-icon :rules="rules">
        <el-form-item
          v-for="(item,index) in form.projectHours"
          :key="index"
          :label="''"
          :label-width="formLabelWidth"
        >
        项目名称：
          <el-select v-model="item.projectId" placeholder="选择项目" >
            <el-option v-for="item in projectArr" :key="item.projectId" :label="item.projectName" :value="item.projectId"/>
          </el-select>
          <el-button type="danger" @click="removeProjectHours(form.projectHours.index)" style="margin-left: 520px;" size="mini"> 删除工时</el-button>
          <div style="display: flex;justify-content: flex-end;margin: 0;">

            <span style="margin:0px 5px;">今日工时</span><el-input
            ...
            ></el-input>
            <span style="margin:0px 5px;">加班工时</span><el-input
              v-model.number="item.workOvertimeHour"
              type="number"
              max="24"
              min="0"
              maxlength="2"
              ...
              @input="lookinput(item.hour)"
            ></el-input>
            <span style="margin:0px 5px;">变更工时</span><el-input
              v-model.number="item.changeHour"
              type="number"
              max="24"
              min="0"
              maxlength="2"
              ...
              @input="lookinput(item.hour)"
            ></el-input>
            <span style="margin:0px 5px;">调休工时</span><el-input
              v-model.number="item.timeOffHour"
              type="number"
              max="24"
              min="0"
              maxlength="2"
              ...
              @input="lookinput(item.hour)"
            ></el-input>
            <span style="margin:0px 5px;">请假工时</span><el-input
              v-model.number="item.leaveHour"
              type="number"
              max="24"
              min="0"
              maxlength="2"
        ...
        <el-button type="success" @click="addProjectHours"> 新增工时</el-button>
...
export default {
  ...
  methods: {
    ...
    addProjectHours(){
      if(null == this.projectArr || this.projectArr.length ==0){
        this.$message.error("请先配置项目");
        return;
      }
      this.form.projectHours.push(
        {projectId:this.projectArr[0].projectId,hour:0,workOvertimeHour:0,changeHour:0,timeOffHour:0,leaveHour:0,daily:''}
      )
    },
    removeProjectHours(index){
      this.form.projectHours.splice(index,1);
    },
