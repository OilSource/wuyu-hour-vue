      ...
      <el-row>
        <el-col :span="24"
          ...
          <el-form ref="form" label-width="80px">
            <el-form-item
              ><label for="">时间：</label>{{ headersDate }}
              <span>
                <el-tag v-if="!dailyInfo.festivals" type="success"
                  >工作日</el-tag
                >
            ...
            <div style="width:50%">
                ...
                <el-descriptions-item label="上报："
                  >{{ fillNumber }}人</el-descriptions-item
                >
                ...
                <el-descriptions-item label="请假："
                  >{{ leaveNumber }}人</el-descriptions-item
                >
            ...
            <el-form-item>
              <el-descriptions title="记录" :column="1">
                <template slot="extra">
                  <el-button type="success" size="mini">批量审批通过</el-button>
                  <el-button type="danger" size="mini">批量审批拒绝</el-button>
                </template>
                <el-descriptions-item>
                  <el-table :data="tableData" stripe style="width: 100%" border >
                    ...
                    <el-table-column prop="nickName" label="人员" width="80">
                    </el-table-column>
                    <el-table-column prop="postName" label="职位" width="120">
                    </el-table-column>
                    <el-table-column prop="everyday" label="所属" width="100">
                    ...
                    <el-table-column prop="fillStatus" label="状态" width="80">
                      <template slot-scope="scope">
                        <div class="">
                          <span v-if="scope.row.fillStatus == 1">上报</span>
                          <span v-else-if="scope.row.fillStatus == 5"
                            >请假</span
                          >
                          <span v-else>调休</span>
                        </div>
                    ...
                    <el-table-column prop="useHour" label="工时" width="50">
                    </el-table-column>
                    <el-table-column prop="workOvertimeHour" label="加班工时" width="90">
                    </el-table-column>
                    <el-table-column prop="changeHour" label="变更工时" width="90">
                    </el-table-column>
                    <el-table-column prop="timeOffHour" label="调休工时" width="90">
                    </el-table-column>
                    <el-table-column prop="leaveHour" label="请假工时" width="90">
                    </el-table-column>
                    <el-table-column prop="daily" label="工作内容" width="300">
                    </el-table-column>
                    <el-table-column prop="approveStatus" :formatter="approveFormat" label="审批状态" width="90">
                    </el-table-column>
                    <el-table-column prop="approveStatus" label="操作" width="150">
                      <template slot-scope="scope">
                        <el-button v-if="scope.row.approveStatus == 0" v-hasPermi="['mh:hour:approve']" type="success" size="mini" @click="doApprove(scope.row.detailId,1)">通过</el-button>
                        <el-button v-if="scope.row.approveStatus == 0" v-hasPermi="['mh:hour:approve']" type="danger" size="mini" @click="doApprove(scope.row.detailId,-1)" >拒绝</el-button>
                      </template>
                    </el-table-column>
                  </el-table>
                </el-descriptions-item>
...
<script>
import { getProjectHourdailyCalendarDeatil ,approveHour} from "@/api/system/project";
export default {
  data() {
    return {
      DialogVisible: false,
  ...
  methods: {
    ...
    approveFormat(row){
      for(let i in this.approveStatusList){
        if(row.approveStatus == this.approveStatusList[i].status){
          return this.approveStatusList[i].label;
        }
      }
    },
    doApprove(detailId,status){
      approveHour({hourId:detailId,status:status}).then(res => {
        console.log(res);
        if (res.code == 200) {
          this.init();
        }
