    ...
    <ul class="calendar-view" v-if="headOptions.showcontent == 1">
      ...
      >
        ...
        <span
          ...
          "
        >
          <el-result v-if="item.mystatus == 1"  >
            <template slot="icon">
              <el-button v-if="getHour(item) >= 8" circle type="success" icon="el-icon-check"></el-button>
              <el-button v-else circle type="warning" icon="el-icon-bell"
              ></el-button>
            </template>
            <template slot="title" >
              <p>已报 {{getHour(item)}} 小时</p>
            </template>
          </el-result>
...
export default {
  ...
  methods: {
    ...
    getHour(row){
      console.log(row);
      let totalHour = 0;
      for(let index in row.projectData){
        totalHour += row.projectData[index].hour;
      }
      return totalHour;
    }
  }
